...
public class EmotionLayout extends LinearLayout implements View.OnClickListener {
     ...
     * 常见表情的设置
     */
    public static final int EMOJI_COLUMNS = 7;//一行表情的个数
    public static final int EMOJI_ROWS = 2;//表情的行数
    ...
    private ViewPager mVpEmotioin_viewpager;
    ...
    private void init() {
        ...
        mVpEmotioin_viewpager = (ViewPager) findViewById(R.id.vpEmotioin);
        dotsLayout = (LinearLayout) findViewById(R.id.llPageNumber);
        bottomTabLayout = (LinearLayout) findViewById(R.id.llTabContainer);
        mRlEmotionAdd = (RelativeLayout) findViewById(R.id.rlEmotionAdd);
        setEmotionAddVisiable(mEmotionAddVisiable);


        initTabs();
        this.findViewById(R.id.emoji_grad_line).setVisibility(GONE);
        bottomTabLayout.setVisibility(GONE);
        mRlEmotionAdd.setVisibility(GONE);
    }
    ...
    private void initTabs() {
        ...
        bottomTabLayout.addView(emojiTab);
        ...
        bottomTabLayout.addView(mSettingTab);
    ...
    private void initListener() {
        if (bottomTabLayout != null) {
            mTabCount = bottomTabLayout.getChildCount() - 1;//不包含最后的设置按钮
            for (int position = 0; position < mTabCount; position++) {
                View tab = bottomTabLayout.getChildAt(position);
                tab.setTag(position);
                tab.setOnClickListener(this);
            }
        }

        mVpEmotioin_viewpager.setOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            @Override
    ...
    private void fillVpEmotioin(int tabPosi) {
        ...
        mVpEmotioin_viewpager.setAdapter(adapter);
    ...
    private void setCurPage(int page, int pageCount) {
        int hasCount = dotsLayout.getChildCount();
        ...
        for (int i = 0; i < forMax; i++) {
            if (pageCount <= hasCount) {
                if (i >= pageCount) {
                    dotsLayout.getChildAt(i).setVisibility(View.GONE);
                    continue;
                } else {
                    ivCur = (ImageView) dotsLayout.getChildAt(i);
                }
            } else {
                if (i < hasCount) {
                    ivCur = (ImageView) dotsLayout.getChildAt(i);
